...
This template shows you how to build a per-transaction billing system with Lago, where a single event can trigger instant charges.
It suits companies whose pricing depends on transactions, such as fintechs and marketplaces that deduct their fees from their customers' revenue.
...
As Stripe supports more than 20 payment methods in the US and most of them share the same charge model, we will focus on card payments.
...
<Steps>
  ...
  <Step title="Create a plan">
    ...
    <CodeGroup>        
      ```bash cURL expandable highlight={7,10,12-39}
      ...
      ```python Python expandable highlight={3-27,34,36-37,40}
      ...
      ```ruby Ruby expandable highlight={3,6,9-35}
      client.plans.create({
        name: 'Payment Processing Plan',
        code: '__PLAN_CODE__',
        interval: 'monthly',
      ...
      ```javascript Javascript expandable highlight={3,6,8,9-35,38}
      const plan = {
        name: 'Payment Processing Plan',
        code: '__PLAN_CODE__',
        interval: 'monthly',
      ...
      ```go Go expandable highlight={3,6,8,9-39,42}
  ...
  <Step title="Ingest transactions via events">
    ...
    2. Use `external_subscription_id` to identify the subscription
...
## Wrap-up
For fintech companies like Stripe, implementing a per-transaction pricing model is a good way to **increase revenue as customers grow and capture value every time a transaction is processed**.
