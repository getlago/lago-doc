...
This template shows you how to build a 'pay-as-you-go' billing system.
It suits companies whose pricing depends entirely on usage, such as cloud service providers and API companies that charge only for the resources their customers consume.
...
<Steps>
  ...
  <Step title="Create a billable metric">
    Create a billable metric to track request usage for the Search API.
    ...
    <CodeGroup>
      ...
      await client.billableMetrics.createBillableMetric({ billable_metric })
      ```
  ...
  <Step title="Create a plan">
    ...
    <CodeGroup>
      ```bash cURL highlight={7,10,13-23}
      ...
      ```python Python highlight={3-11,15,18,21,24}
      ...
      ```javascript JavaScript highlight={3,6,9-19,22}
      const plan = {
        name: 'Algolia Search PAYG',
        code: '__PLAN_CODE__',
        interval: 'monthly',
