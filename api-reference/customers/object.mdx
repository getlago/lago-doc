...
<ResponseField name="customer" type="object">
  <Expandable title="object" defaultOpen="true">
    ...
    <ResponseField name="sequential_id" type="integer" required>
      ...
      customer has a distinct `sequential_id` associated with them, allowing for
      easy identification and sorting based on the order of creation.
    </ResponseField>
    ...
    <ResponseField name="billing_configuration" type="object">
      <Expandable title="object">
        ...
        <ResponseField name="provider_payment_methods" type="array">
          Specifies the available payment method(s) that can be used for this
          customer when `payment_provider` is set to `stripe`. The
          `provider_payment_methods` field is an array that allows multiple
          payment options to be defined. If this field is not set by the user,
          `card` is selected by default. Possible values include `card`,
          `sepa_debit`, `us_bank_account`, `bacs_debit` or `link`.
    ...
    <ResponseField name="shipping_address" type="object">
      <Expandable title="object">
        ...
        <ResponseField name="zipcode" type="string">
          The zipcode of the customer's shipping address.
        </ResponseField>
        <ResponseField name="state" type="string">
          The state of the customer's shipping address.
        </ResponseField>
        <ResponseField name="country" type="string">
          Country code of the customer's shipping address. Format must be ISO
          3166 (alpha-2).
        </ResponseField>
      </Expandable>
    </ResponseField>
    ...
    <ResponseField name="taxes" type="array">
      <Expandable title="array">
        ...
        <ResponseField name="plans_count" type="integer" required>
          Number of plans this tax is applied to (directly or via the
