...
<ParamField path="subscription.canceled" type="data.object">
  Sent when an `incomplete` subscription is canceled before it ever activated. The `cancellation_reason` field indicates why: `payment_failed` (the payment was declined), `timeout` (the activation timeout elapsed without a confirmed payment), or `manual` (the subscription was canceled on demand while still incomplete, via a terminate request with `status=incomplete`).
