...
Along with the payload the message contains both `X-Lago-Signature` and `X-Lago-Signature-Algorithm` HTTP header.
...
You can choose between two different signature algorithms during webhook endpoint creation, `hmac` or `jwt`.
