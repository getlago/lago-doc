...
```bash
brew install getlago/tap/lago   # or: go install github.com/getlago/lago-cli/cmd/lago@latest
```

**Configure and call**

```bash
# --region us | eu | self-hosted (with --api-url). Validates the key before saving a profile.
lago init --api-key "__YOUR_API_KEY__" --region us --mode test

lago customers list --limit 5
lago api GET /customers?page=2
```

Every endpoint in this reference has a generated command. See the [Lago CLI guide](/guide/lago-cli/overview) and the [command reference](/guide/lago-cli/commands/overview).
