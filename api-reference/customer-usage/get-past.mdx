...
namespace Example
{
  public class FindCustomerPastUsageExample
  {
      public static void Main()
      {
          ...
          try
          {
              // Find customer past usage
              CustomerUsage result = apiInstance.FindCustomerPastUsage(customerExternalId, externalSubscriptionId);
              Debug.WriteLine(result);
          }
          catch (ApiException e)
          {
              Debug.Print("Exception when calling CustomersApi.FindCustomerPastUsage: " + e.Message );
...
try {
  $result = $apiInstance->findCustomerPastUsage($customer_external_id, $external_subscription_id);
  print_r($result);
} catch (Exception $e) {
  echo 'Exception when calling CustomersApi->findCustomerPastUsage: ', $e->getMessage(), PHP_EOL;
}
```
