...
5. Select **"Issue a credit note"** from the **"Actions"** dropdown (upper right
   corner).
...
<Info>
  Coupons and prepaid credits cannot be refunded, they can only be credited back
...
In addition to this, a `credit_note.created`
[webhook](/docs/api/webhooks/messages) will automatically be sent by Lago.
...
in the `credit_note.created` [webhook](/docs/api/webhooks/messages), which will
allow you to proceed with the payment.
...
`credit_note.created` [webhook](/docs/api/webhooks/messages) and displayed on
the credit note details page.
