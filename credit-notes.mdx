...
In addition to this, a `credit_note.created` [webhook](/api/webhooks/messages)
...
in the `credit_note.created` [webhook](/api/webhooks/messages), which will allow
...
`credit_note.created` [webhook](/api/webhooks/messages) and displayed on the
...
<Info>
  ...
  Credit note wallets are different from wallets associated with [prepaid credits](/prepaid-credits).
