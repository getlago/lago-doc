...
Issue credit note to refund or credit back to your customer's account a `finalized` invoice.
...
<Info>
  Credit notes can only be issued for prepaid credit invoices with a `succeeded`
  payment status. These credit notes are refundable only for this type of
  invoice and will directly void prepaid credits in the associated wallet.
</Info>

<Info>
  Prepaid credits or existing credit notes applied to a subscription invoice
  cannot be refunded; they can only be credited back to the customer’s account.
  Coupons can neither be refunded nor credited back to the customer’s account.
</Info>
