...
Each Lago account is associated with a single environment. It is not possible
to have your test environment and production environment in the same account.
...
Both workarounds require you to replicate the same setup in each
environment.
...
A success toast appears and the new subscription is listed in the customer view.
...
Lago automatically aggregates the events your backend sends according to the
billable metrics you defined ([learn more about ingesting usage](/guide/events/ingesting-usage)):
...
At the end of the billing period, Lago automatically generates an invoice for
each customer and notifies your application with a webhook. To define the URL
that receives them:

1. In the side menu, select **"Developers"**;
2. Open the **"Webhooks"** tab;
3. Click **"Add a webhook"**;
4. Enter the name and the URL of your application; and
5. Click **"Add webhook"** to confirm.

To learn more, see [Webhooks](/guide/webhooks) and the
[invoice object](/api-reference/invoices/invoice-object).
