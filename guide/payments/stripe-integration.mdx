...
- You must manually retry using the [payment URL endpoint](/api-reference/invoices/payment-url)
...
2. **Automatic retry** (within seconds): New payment attempt with 3DS enabled
   ...
   - Lago sends `payment.requires_action` [webhook](/api-reference/webhooks/messages#param-payment-requires-action)  containing the 3DS authentication URL
