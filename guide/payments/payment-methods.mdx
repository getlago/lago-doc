...
<Note>
**Manual payment does not exempt an invoice from dunning.** It only stops the automatic charge when the invoice is finalized. If that invoice later becomes overdue and the customer is enrolled in a dunning campaign, Lago can still attempt an automatic payment for the overdue balance through the customer's payment provider. This also applies to a billing object that was switched to **Manual payment** because its payment method became inactive. See [automatic dunning](/guide/dunning/automatic-dunning) to learn how to exclude a customer from these attempts.
</Note>
