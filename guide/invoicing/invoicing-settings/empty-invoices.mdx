...
By default, Lago finalizes all invoices, including those with a total amount of zero. However, some customers may prefer to skip these invoices to avoid generating unnecessary accounting documents.
...
You can disable it at the billing entity level under Invoice settings. When set to `false`, Lago skips finalizing empty invoices for all customers linked to that billing entity, unless a customer-specific override is applied.
...
<Tabs>
  ...
  <Tab title="API">
    <CodeGroup>
    ```bash Update the billing entity's settings
...
### Customer level
By default, new customers inherit the `finalize_zero_amount_invoice` setting from the billing entity.
