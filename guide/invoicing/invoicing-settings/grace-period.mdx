...
<Tabs>
  <Tab title="Dashboard">
      ...
      <Warning>
        If you shorten the billing entity's grace period, some invoices in `draft`
  ...
  <Tab title="API">
    You can also modify the entity's grace period at any time through [the API](/api-reference/billing-entities/update).
      <CodeGroup>
      ```bash Add a grace period on the billing entity {28}
...
See [draft invoices](/guide/invoicing/draft-invoices) for details on each of these actions.