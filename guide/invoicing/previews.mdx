...
Lago generates a preview invoice on demand, prorating all line items based on the `billing_time` (calendar or anniversary).
...
If you have an external tax setup, you can include tax details in the invoice preview payload. Lago then calculates and applies taxes accordingly. 
...
2. **Provide coupon details in the request payload:** Pass your own coupon information directly in the invoice preview payload, and Lago applies the discount accordingly.
