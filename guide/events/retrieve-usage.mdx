...
<Tabs>
  <Tab title="Dashboard">
    The current usage endpoint also provides an estimate of usage and billing amounts projected through the end of the current billing period. 
    It uses current consumption trends and billing configuration to forecast the likely totals at period close. Projected usage leverages the same underlying [endpoint as current usage](../../api-reference/customer-usage/get-current) to ensure consistency in data structure and interpretation.
