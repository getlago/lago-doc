...
- **No pricing group keys**: the breakdown sits at the charge level, in a `presentation_breakdown` array:
    ...
    "charges_usage": [
      {
        ...
        "presentation_breakdown": [
          { "presentation_by": { "department": "eng" },   "units": "60.0", "display_in_invoice": true },
          { "presentation_by": { "department": "sales" }, "units": "40.0", "display_in_invoice": true }
        ]
      }
    ]
  }
...
<Tip>
  Pass the optional `breakdown_by` query parameter to limit the response to a subset of the configured presentation keys. For example, `breakdown_by[]=region` collapses a `region · city` breakdown down to region only.
</Tip>
