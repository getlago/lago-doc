...
<Info>
  S3 imports are **idempotent**: re-importing a file after failure is always safe (dedup by `transaction_id`). Timestamps can be historical — Lago assigns events to the correct billing period retroactively.
</Info>
