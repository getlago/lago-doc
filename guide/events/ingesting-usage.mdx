...
The SDK emits one event per non-zero token dimension (input, output, cached input, cache creation, tool calls, etc.) and maps each to a default Lago metric code. See the [Agent SDK guide](/guide/ai-agents/agent-sdk) for the full list of dimensions, configuration options, and multi-tenant subscription routing.
