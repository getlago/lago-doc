...
<AccordionGroup>
  <Accordion title="transaction_id (required)">
    A unique ID you generate. Lago uses it for **deduplication**: if the same event arrives twice, it is billed once. Which copy Lago keeps depends on your event store — see Idempotency and deduplication.
    ...
    <Warning>
      ...
      - If the `transaction_id` has been received but with a different `timestamp`, the two events are distinct and both are billed
      Always send `timestamp` explicitly on this pipeline. When you omit it, Lago sets it to the ingestion time, which differs on every call — so a retry without an explicit `timestamp` is stored and billed as a separate event.
    </Warning>
  </Accordion>
  ...
  <Accordion title="external_subscription_id (required)">
    ...
    If the `external_subscription_id` does not match an active subscription in Lago, the event is ingested but not post-processed.
  </Accordion>
  ...
  <Accordion title="code (required)">
    ...
    If the `code` does not match a billable metric in Lago, the event is ingested but not post-processed.
  </Accordion>

  <Accordion title="timestamp (optional)">
    ...
    <Info>
      If you do not specify a timestamp, Lago automatically uses the reception time of the event.
    </Info>
  </Accordion>

  <Accordion title="properties (optional)">
    Key-value pairs carrying the data your pricing needs: token counts, regions, instance types, anything you price on. Properties can be any JSON supported datatype including `strings`, `integers`, `floats`, `uuids`, or `timestamps`.
  ...
  <Accordion title="precise_total_amount_cents (optional)">
    Skip Lago's aggregation and set the monetary amount directly. This value must be a **string** to avoid floating-point rounding issues.
...
<CardGroup cols={2}>
  ...
  <Card title="Test with a single event first">
...
| **Agent SDK** | LLM token billing. Wraps your provider client (Bedrock, Mistral, etc.) and emits token events automatically. |
...
<Info>
  Replace `__LAGO_API_URL__` with `api.getlago.com` or `api.eu.getlago.com` for Lago Cloud, or your own instance URL for self-hosted deployments.
</Info>

<Tabs>
  <Tab title="Single event">
    ```bash
    LAGO_URL="https://__LAGO_API_URL__"
  ...
  <Tab title="Batch (up to 100)">
    ...
    <Warning>
      **Batches are atomic.** If a single event is invalid, Lago rejects the entire batch with a `422` and persists nothing. The valid events in the same request are dropped too. "Invalid" covers a missing `transaction_id` or `code`, a `timestamp` Lago cannot parse (`invalid_format`), a custom expression that fails to evaluate, and — on the Postgres event store only — a duplicate `transaction_id` (`value_already_exist`). The response reports failures by the event's **index in the array you sent**, not by `transaction_id`. Keep your array order so you can trace an index back to its source event. Validate and de-duplicate before batching.

      An `external_subscription_id` that matches no subscription does not fail the batch: those events are ingested and skipped at post-processing. On the ClickHouse event store, duplicate `transaction_ids` do not fail the batch either — they are accepted with a 200 and resolved by deduplication instead. Don't rely on a 422 to catch duplicates; de-duplicate before you send.
    </Warning>
  </Tab>
</Tabs>
...
For applications that bill on LLM tokens, the [Agent SDK](/guide/ai-agents/agent-sdk/overview) is the lowest-effort path to ingestion. You wrap your existing provider client and keep calling it the same way. The SDK extracts token usage from each response, normalizes it across providers, and ships events to Lago in the background.

- **Providers:** AWS Bedrock (`Converse` and `InvokeModel`, sync and stream), Mistral, OpenAI, Anthropic, Google Gemini, and potentially more.
- **Languages:** [Python](https://github.com/getlago/lago-agent-sdk-python) and [JavaScript / TypeScript](https://github.com/getlago/lago-agent-sdk-js)).
...
<CodeGroup>
  ...
  sdk = LagoSDK(api_key="__YOUR_API_KEY__", default_subscription_id="sub_42")
...
The SDK emits one event per non-zero token dimension (input, output, cached input, cache creation, tool calls, etc.) and maps each to a default Lago metric code. See the [Agent SDK guide](/guide/ai-agents/agent-sdk/overview) for the full list of dimensions, configuration options, and multi-tenant subscription routing.

### Kafka / Redpanda configuration

Kafka ingestion requires the **ClickHouse event store** (see [Architecture](/guide/events/usage-performances#architecture-how-lago-processes-events-at-scale)). Self-hosted instances enable it with `LAGO_CLICKHOUSE_ENABLED` and `LAGO_DEFAULT_EVENT_STORE=clickhouse` — see [Configuring the event store](/guide/lago-self-hosted/docker#configuring-the-event-store).
...
<AccordionGroup>
  ...
  <Accordion title="Reader / producer tuning">
    Lago can directly consume events from your Kafka topic (which will require opening access to your Kafka broker / topic). Alternatively, you can send your events to a dedicated topic on our infrastructure (AWS VPC Peering or PrivateLink).
...
Lago deduplicates re-ingested events through the `transaction_id`, so a re-sent event is billed once. Deduplication works **across delivery methods**: a REST event isn't billed again if the same ID arrives via Kafka.

The deduplication key and the resolution rule depend on which event store your organization runs on:

| | Postgres event store | ClickHouse event store |
|---|---|---|
| Deduplication key | `transaction_id` + `external_subscription_id` | `transaction_id` + `timestamp` + `external_subscription_id` + `code` |
| Repeat ingestion | Rejected with a `422` (`value_already_exist`) | Accepted with a `200`; the latest copy **replaces** the earlier one |
| Which copy is billed | The first one received | The most recently ingested one |

<Warning>
  On the ClickHouse event store, the `timestamp` is part of the deduplication key. A retry that carries the same `transaction_id` but a **different** `timestamp` is a distinct event and **is billed again**. Because Lago defaults `timestamp` to the ingestion time when you omit it, every retry of a timestamp-less event produces a new billable event.

  **Send `timestamp` explicitly, and resend the original payload verbatim on retry.** Done that way, retries are safe on both stores: REST timeout? Resend. Kafka consumer restart? Reprocess. S3 import interrupted? Re-trigger.
</Warning>

Charges billed **in advance** are protected separately: Lago deduplicates their fees on `transaction_id` alone, so a duplicate can't produce a second in-advance fee on either store, regardless of `timestamp`.
...
<AccordionGroup>
  <Accordion title="Late-arriving events">
    Events that arrive after a billing period has closed are assigned to the correct historical period based on `timestamp`. If the invoice has already been finalized, the event will not be included in the billing cycle. However if they match a recurring billable metric, they will impact the next billing cycle.
  </Accordion>

  <Accordion title="Correcting events">
    - With Postgres: duplicates are rejected with a 422 and there is no replacement. Use a new transaction_id to correct an event.
    - With ClickHouse: send a new event with the same transaction_id and the same timestamp, carrying the updated properties. The new event replaces the original. A different timestamp creates a second billable event instead of amending the first.
  </Accordion>

  <Accordion title="Events before subscription start">
    Events with a `timestamp` before the subscription's `started_at` date are ingested, but ignored during post-processing and aggregation. Validate your timestamps if you're backfilling historical data.
  </Accordion>
  ...
  <Accordion title="Events for terminated subscriptions">
    ...
    You can still send usage events with timestamp before the subscription `terminated_at` if you want to amend a draft invoice.
  </Accordion>
</AccordionGroup>
...
**Test with a single event first.** Send one event via the REST API, then retrieve it with `GET /events/{transaction_id}` to confirm Lago received and parsed it correctly.
...
- **REST API response codes:** 200 = accepted, 422 = validation error (missing transaction_id or code, invalid timestamp, failing expression, or a duplicate transaction_id on the Postgres event store), 429 = rate limited (retry safely), 5xx = server-side error (investigate before retrying)
...
Lago enforces per-organization rate limits on the REST API, with the highest limits on event ingestion. If you exceed them you receive a `429 Too Many Requests` response with `X-RateLimit-Limit`, `X-RateLimit-Remaining`, and `X-RateLimit-Reset` headers. Retrying after a `429` is safe as long as you resend the identical payload, timestamp included (see Idempotency and deduplication). See [Rate limits](/api-reference/rate-limits) for the full limits and retry guidance.
