...
<Tabs>
  ...
  <Tab title="Graduated percentage">
      ...
      apply a **flat fee** per tier and a **fixed fee** per transaction (per event) in addition to the rate.
      ...
      | Tier   | First unit | Last unit | Rate       | Fixed fee | Flat fee |
      | ------ | ---------- | --------- | ---------- | --------- | -------- |
      | Tier 1 | $0         | $1000     | 1%         | $0.5      | $200     |
      | Tier 2 | $1,001     | $10,000   | 2%         | $0.4      | $300     |
      | Tier 3 | 10,001     | ∞         | 3%         | $0.3      | $400     |

      A first transaction at $500 would cost (500 units x 1%) + ($0.50 fixed fee x 1 transaction) + $200 flat fee = $205.50.
      A second transaction at $550 would cost [(500 units x 1%) + ($0.40 fixed fee x 1 transaction)] + [(50 units x 2%) + $300 flat fee] = $306.40.
      A third transaction at $4000 would cost (4000 x 2%) + ($0.40 fixed fee x 1 transaction) = $80.40

      <Tip>
        **The fixed fee applies to individual events, each representing a transaction.** 
        When a transaction spans two tiers, the Lago billing engine only charges the fixed fee of the higher tier.
      </Tip>
