...
Plans can be fully edited as long as they are not linked to a customer (i.e., no active [subscriptions](/guide/subscriptions/assign-plan)). Once a plan is assigned to a customer, you are able to modify the prices of the subscription or charge fees. Other settings (such as "plan interval", “pay in advanced”, pro-rated charges …) cannot be edited once they are assigned to a customer.

To update Plans that are already linked to customers, you will need to either remove all active subscriptions or create an entirely new plan.

To update a specific charge in a Plan, it must be removed and re-added with the updated details.
