...
At the end of the period, the invoice will generate **one fee of $3.45** (see example below).

| Item               | Unit | Unit price | Amount |
| ------------------ | ---- | ---------- | ------ |
| Dedicated Instance | 22   | $0,156818  | $3.45  |

To display more detailed information on fees, please refer to the [custom invoice grouping](/guide/plans/charges/grouping#presentation-group-keys) or the [pay in advance](/guide/plans/charges/arrears-vs-advance#charges-paid-in-advance) option.

<Note>

Only the `standard` & `dynamic` charge models support the [custom invoice grouping](/guide/plans/charges/grouping#presentation-group-keys).
