...
|---|---|---|---|
| **Runs in** | claude.ai or Claude Code | Claude managed agent | Your infrastructure |
...
<Tabs>
  <Tab title="Skill">
    Upload the skill folder under **Settings → Capabilities → Skills**, enable the Lago, Gmail, and Slack connectors, then ask *"who owes me money"*.

    It creates Gmail drafts and never sends. Memory lives in three customer metadata keys: `dunning_score`, `dunning_contact`, and `dunning_hold`. Clear `dunning_hold` to hand an account back to the agent.

    <Warning>
    Metadata writes must merge. Resend existing items with their id, or Lago rejects the duplicate key and deletes anything you left out.
    </Warning>
  </Tab>
...
|---|---|
| **Lago** | The billing truth. Reads invoices, payments, disputes, portal links. One write: the retry. |
