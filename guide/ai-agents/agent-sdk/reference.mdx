...
| `ramp_router_api_key`     | `rampRouterApiKey`        | `None` / `undefined`             | Usually unnecessary — wrapping an OpenAI or Anthropic client pointed at Ramp Router reads the key it already carries. Set it only when pricing Router usage without ever calling `wrap()`; an explicit value always wins over a learned one |
...
The `where` argument names the phase that failed: `emit`, `send_batch`, `pricing`, one of `pricing.fetch_openrouter` / `pricing.fetch_bedrock` / `pricing.fetch_cloudflare_workers_ai` / `pricing.fetch_mistral_aliases` / `pricing.fetch_ramp_router`, or `overflow` (JavaScript only).
