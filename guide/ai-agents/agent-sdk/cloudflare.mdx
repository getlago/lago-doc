...
<CardGroup cols={2}>
  <Card title="Live path" icon="bolt">
    Point your existing wrapped client at the gateway. `wrap()` detects it and adjusts. Bills as calls happen.
  </Card>
  <Card title="Backfill path" icon="clock-rotate-left">
    Poll the gateway's Logs API and bill Cloudflare's own metered cost. Idempotent, so re-running a window never double-bills.
  </Card>
</CardGroup>

<Warning>
  **This is not released yet.** Cloudflare support lands in [lago-agent-sdk-python#13](https://github.com/getlago/lago-agent-sdk-python/pull/13) and [lago-agent-sdk-js#28](https://github.com/getlago/lago-agent-sdk-js/pull/28), both open at the time of writing. Install from the branch until they ship.
</Warning>
...
### Gateway cache hits are not billed

When the gateway serves a response from its own cache, it sets `cf-aig-cache-status: HIT` and never calls the provider. Nothing was spent, so the SDK emits nothing for that response.
...
<Warning>
  **Cache-hit detection covers OpenAI and Anthropic clients, non-streaming only.**
  ...
  If you rely on gateway caching for cost control, use the backfill path. It reads the `cached` flag off every log entry, for every provider, streaming included.
</Warning>

### Workers AI is priced automatically
