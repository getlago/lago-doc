...
The check reads the response header through the provider SDK's raw-response accessor. With no gateway in the path the header is simply absent, so this is a no-op on direct provider calls.

<Warning>
  **Cache-hit detection covers non-streaming calls only.** Streamed responses do not expose the response headers to the wrapper, so a streamed cache hit is billed as a normal call. If you rely on gateway caching for cost control, prefer non-streaming for cacheable requests, or use the backfill path, which reads the `cached` flag straight off the log entry.
</Warning>
...
| Price source | OpenRouter, AWS, or Cloudflare's catalog | Cloudflare's own metered `cost` |
| Cache hits | Skipped, non-streaming only | Skipped, read from the `cached` flag |
| Streaming | Billed, cache hits included | Handled correctly |
