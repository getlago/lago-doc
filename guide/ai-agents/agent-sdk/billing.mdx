...
| You use Bedrock Claude, Databricks or Snowflake Cortex, which the SDK always bills in tokens | You want the cost broken down by token type on the invoice |
...
- **Cloudflare's own model catalog** for Workers AI. Needs a Cloudflare account id and API token — see [Cloudflare AI Gateway](/guide/ai-agents/agent-sdk/cloudflare#workers-ai-is-priced-from-cloudflares-own-catalog).
- **Ramp Router's own model catalog** for calls through Router: the rates Router bills, rather than a public listing. The catalog is account-scoped, and `wrap()` reads the Router key from the client you pass in — see [Ramp Router](/guide/ai-agents/agent-sdk/ramp-router#price-mode).
