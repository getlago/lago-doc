...
| You use Bedrock Claude, Databricks, Snowflake Cortex or Ramp Router, where price data is missing or deliberately unmatched | You want the cost broken down by token type on the invoice |
...
**Ramp Router is always billed as token counts.** Router serves several vendors behind one endpoint, and its effective rate is unknowable from the response: a request served with your own provider key costs $0 through Router, and a non-default service tier does not bill at the published base rate. Router also normalizes token-overlap numbers to OpenAI's convention rather than the served vendor's, so stamping the served vendor as the provider would de-overlap the counts with the wrong rules. A price-mode caller gets token events here with no price-miss error. See [Ramp Router](/guide/ai-agents/agent-sdk/ramp-router).
