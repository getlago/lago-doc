...
Use it for data and reporting only. To *change* something in natural language, use the [Billing Assistant](/guide/ai-agents/billing-assistant). It can perform destructive actions, such as terminating subscriptions or issuing refunds.
...
Gate access at the assistant level, in each user's permissions, not through role-based data restrictions that the assistant does not enforce.
...
- **Results are AI-generated.** They can be wrong. Review before you rely on them, and cross-check against your [Analytics](/guide/analytics/overview) dashboards or the API for anything you report externally.
...
- [Analytics](/guide/analytics/overview): prebuilt finance and usage dashboards.
