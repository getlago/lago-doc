...
Purchased credits are added to the customer's wallet only after payment succeeds.

The invoice uses the [wallet's payment terms](/guide/wallet-and-prepaid-credits/overview#payment-terms), falling back to the customer's or billing entity's terms when the wallet has none of its own. This applies to manual and recurring credit purchases.

For example, a top-up invoice dated **April 10, 2026** with **Net 15** is due **April 25**. The credits become available when payment is confirmed, even if the customer pays before that deadline. With **Due on receipt**, the same invoice is due **April 10**. See [all payment-term options](/guide/invoicing/invoicing-settings/payment-term#choose-a-payment-term).
