...
<Tabs>
  <Tab title="Dashboard">
    ...
    10. Define a PO number to be used on invoices generated from this wallet.
    11. Set the [expiration date](#expiration-date-and-termination) (optional);
    12. In the payment settings, keep the inherited [payment term](#payment-terms) or choose one for this wallet; and
    13. Click **"Add wallet & credits"** to confirm.

    <Info>
      The currency of the wallet can differ from the default currency of the customer.
      Prepaid credits only apply to invoices sharing the same currency as the wallet.
    </Info>
...
Free credits are added to the customer's wallet instantly, while purchased credits are added when payment is confirmed.

## Payment terms

A wallet's payment terms determine when invoices for its purchased credits are due. They apply to credit purchases, including manual and recurring top-ups. They do not set the payment terms of subscription invoices that use those credits.

When creating or editing the wallet, open its payment settings. Keep the inherited customer term, or choose a payment-term type and its days or month settings, then save. If the customer has no term of their own, the billing entity's default applies. To return to the default later, choose inheritance.

For example, your customer uses **Net 30** for subscriptions, but you set their wallet to **Due on receipt**. A top-up invoice dated **April 10, 2026** is due **April 10**; a subscription invoice dated the same day is due **May 10**.

See [Payment terms](/guide/invoicing/invoicing-settings/payment-term#choose-a-payment-term) for all six options and examples. Changes apply to future invoices; existing invoices retain their stored term.

<Info>
  Giving a customer more time to pay does not make purchased credits available before payment. For example, a Net 15 top-up invoice dated April 10 is due April 25, but the credits are added only when payment is confirmed. The [invoicing after successful payment option](#invoicing-after-successful-payment-option) separately controls when the invoice is issued.
</Info>
...
2. **Ongoing Balance:** balance accounted for current usage, including taxes and draft invoices offering a real-time estimate of your balance's consumption. It refreshes every minute, or whenever an invoice is finalized for the customer.
...
<Tabs>
  <Tab title="Dashboard">
    ...
    <Frame caption="Wallet transaction limits">
      <img src="/guide/images/min-max-wallet-limits.png" />
    </Frame>

  </Tab>
  <Tab title="API">
    ```bash Add wallet min and max limits
    LAGO_URL="https://api.getlago.com"
    API_KEY="__YOUR_API_KEY__"
  ...
  This applies to both single transactions and automatic top-ups (recurring wallet transactions).
    ...
    <Frame caption="Skip wallet limits for a single transaction">
      <img src="/guide/images/skip-wallet-limits.png" />
    </Frame>
