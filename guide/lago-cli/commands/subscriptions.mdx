...
Every body field sits under `subscription`, so every flag carries the `--subscription-` prefix.

| Flag | Description |
| --- | --- |
| `--subscription-external-customer-id` | The customer. |
| `--subscription-external-id` | Your subscription identifier. Reusing it changes the plan. |
| `--subscription-plan-code` | The plan. |
| `--subscription-billing-time calendar\|anniversary` | Period alignment. |
| `--subscription-subscription-at`, `--subscription-ending-at` | Start and end, ISO 8601. |
| `--subscription-plan-overrides-*` | Negotiated terms for this customer only: `amount-cents`, `charges`, `trial-period`, `minimum-commitment-amount-cents`. |
