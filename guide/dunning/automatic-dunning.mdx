...
To stop a customer who always pays manually from being charged this way, you have two options:
- Leave the **default payment provider empty** on the customer. Dunning still sends the payment request email, but no payment is attempted.
- Enable **Exclude from dunning campaign** on the customer. This stops the automatic payment and the dunning emails, so the customer receives no reminders at all.
</Note>
