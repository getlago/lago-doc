...
5. **Canceled on demand** before the payment resolves → the subscription becomes `canceled` (reason: `manual`). See [Cancel an incomplete subscription](/guide/subscriptions/terminate-subscription#cancel-an-incomplete-subscription).
...
- **Editing is locked, but you can cancel.** You can't edit, upgrade, or downgrade an `incomplete` subscription. You can [cancel it on demand](/guide/subscriptions/terminate-subscription#cancel-an-incomplete-subscription) (reason: `manual`); otherwise it stays incomplete until its rule is satisfied, fails, or times out.
