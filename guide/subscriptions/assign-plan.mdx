...
<Tabs>
  <Tab title="Dashboard">
    ...
    4. Select a plan (that you can overwrite if needed - [see below](#overriding-a-plan));
    ...
    9. Define a PO number to be used on invoices generated from this subscription.
    10. Choose whether the subscription should be renewed at the beginning of the period or on its anniversary date (see [below](#billing-cycles));
    11. In the payment settings, keep the inherited [payment term](#payment-terms) or choose one for this subscription; and
    12. Click **"Add plan"** to confirm.
  </Tab>
  <Tab title="API">
    <CodeGroup>
...
Payment terms determine when the subscription's invoices are due. When assigning a plan, open the payment settings and choose a term for this subscription, or keep the inherited customer term. If the customer has no term of their own, the billing entity's default applies.

For example, keep **Net 30** for a standard subscription and choose **Net 60** for an enterprise subscription belonging to the same customer. For invoices dated **April 10, 2026**, payment is due on **May 10** and **June 9**, respectively. The different terms require [separate invoices](#invoice-consolidation).

See [Payment terms](/guide/invoicing/invoicing-settings/payment-term#choose-a-payment-term) for all six options and dated examples. You can also [change an existing subscription's term](/guide/subscriptions/edit-subscription#change-payment-terms).
...
<Warning>
When assigning multiple plans to a customer, you must specify the `external_subscription_id` for each event or create [batch events](/api-reference/events/batch).
</Warning>
...
When a customer is subscribed to multiple plans, Lago can either group all subscription charges into a **single consolidated invoice** or issue a **separate invoice for each subscription**.

For subscriptions to share an invoice, they must be billed on the same day, share the same payment method, and have the same effective payment term. Different payment terms require separate invoices, even if they happen to produce the same due date for a particular month.

The effective term is the term that applies after checking subscription, customer, and billing-entity settings. For example, a subscription that inherits Net 30 and one explicitly set to Net 30 have the same effective term. A Net 30 subscription and a Net 60 subscription do not.

This reduces the number of invoices your customers receive and simplifies payment reconciliation. In the example below, a customer is subscribed to three plans — two billed monthly and one billed yearly. All three use the same payment method and payment term, and Lago groups eligible charges that fall on the same billing date into a single invoice:
...
If you'd prefer to keep a specific subscription on its own invoice — for example, to bill a different cost center, track a contract independently, or simplify internal reporting — you can disable consolidation for that subscription. To do so, set the `consolidate_invoices` field to `false` on the subscription, either from the dashboard or via the API. Lago will then issue a standalone invoice for that subscription, even when other subscriptions for the same customer are billed on the same day.

<Info>
The `consolidate_invoices` setting applies at the **subscription level**, so you can mix consolidated and standalone subscriptions for the same customer. You can update this setting at any time.
</Info>
