...
<Tabs>
  <Tab title="Dashboard">
    ...
    10. Choose whether the subscription should be renewed at the beginning of the period or on its anniversary date (see [below](#billing-cycles)); and
    11. Click **"Add plan"** to confirm;
  </Tab>
  <Tab title="API">
    <CodeGroup>
...
By default, Lago automatically consolidates subscriptions into a single invoice when they meet both of these conditions:
- They are billed on the same day; and
- They share the same payment method.

This reduces the number of invoices your customers receive and simplifies payment reconciliation. In the example below, a customer is subscribed to three plans — two billed monthly and one billed yearly — and Lago groups all charges that fall on the same billing date into a single invoice:
