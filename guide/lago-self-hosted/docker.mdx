...
|-------------------------------------------------------------------------------------------|--------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `DATABASE_URL`                                                                            | —                  | Full Postgres URL used by the API, workers and clock containers (`postgresql://user:password@host:port/database`). **Required in production** and overrides any `POSTGRES_*` variable.        |
...
<Info>
  If both `DATABASE_URL` and individual `POSTGRES_*` variables are set, `DATABASE_URL` wins. We recommend defining `DATABASE_URL` explicitly in any non-docker-compose deployment so the connection string is unambiguous and easy to rotate.
</Info>
...
| Variable                       | Default | Purpose                                                                                                                       |
|--------------------------------|---------|-------------------------------------------------------------------------------------------------------------------------------|
| `DATABASE_POOL`                | `10`    | Maximum number of connections each API, worker and clock instance opens to Postgres.                                          |
| `DATABASE_PREPARED_STATEMENTS` | `true`  | Toggles Postgres prepared statements. Set to `false` when fronting Postgres with a transaction-mode pooler such as PgBouncer. |
...
These three opt-in variables are applied to every Rails connection (API, workers, clock, events) via the shared `default` block in Lago's `database.yml`. They are unset by default — Postgres runs with no per-statement cap.
...
| Sidekiq workers     | unset                             | unset                           | unset                        |
...
- **Use `DATABASE_URL` as the single source of truth.** It is the only variable read by the API in production; `POSTGRES_*` are ignored when you do not use the bundled container.
...
- **Disable prepared statements behind a transaction pooler.** Set `DATABASE_PREPARED_STATEMENTS=false` when running PgBouncer (or similar) in transaction mode.
