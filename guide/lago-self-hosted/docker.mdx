...
- **ClickHouse** - columnar storage for high-volume ingestion, sub-second aggregation over billions of events, and a prerequisite for [Kafka/Redpanda ingestion](/guide/events/ingesting-usage#kafka--redpanda-configuration). See [Performance & Optimization](/guide/events/usage-performances) for the architecture and benchmarks.
