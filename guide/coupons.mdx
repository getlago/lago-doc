...
<Tabs>
  ...
  <Tab title="API">
    ...
    ```bash Create a percentage coupon
    LAGO_URL="https://api.getlago.com"
    API_KEY="__YOUR_API_KEY__"
...
<Info>
  A coupon can be limited to plans or to billable metrics, not both. Setting
  both returns an `only_one_limitation_type_per_coupon_allowed` error.
</Info>
...
<Info>
You can apply several coupons to a customer. A non-reusable coupon can only be
applied once to a customer account, even if it has not been fully consumed. A coupon applied to a customer continues to apply beyond the expiration date.
</Info>

Coupons are deducted from future subscription invoices, before tax.

For coupons whose value is a fixed amount:

- **When the coupon only applies once** and its value is higher than the invoice
  amount, the remainder carries over to the following invoices until the coupon
  is fully consumed or removed; and
- **When the coupon is recurring** and its value is higher than the invoice amount,
  any remaining unused amount is lost, even if it is the last application
  period.

When several coupons are applied to the customer account, Lago deducts them in this order:
...
<Info>
  Once a coupon has been applied to a customer, you can only edit its name,
  description and expiration settings. The code, type, value, frequency,
  reusability and plan or metric limitations are locked.
</Info>
...
The coupon disappears from the customer view immediately and is not applied to
the next invoice.
