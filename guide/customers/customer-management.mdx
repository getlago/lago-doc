...
The "**Billing overview**" lets you see in a glance where a customer stands in terms of revenue and collection. It looks at the past 12 months.
...
<Tabs>
  <Tab title="Dashboard">
   <Frame caption="Fetch customer's gross revenue">
      <img src="/guide/images/customer-gross-revenue.png" />
    </Frame>
  </Tab>
  <Tab title="API">
   You can retrieve the customer's gross revenue via the API using [this endpoint](/api-reference/analytics/gross-revenue).
      ...
      curl --location --request GET "$LAGO_URL/api/v1/analytics/overdue_balances?external_customer_id=hooli_1234&currency=USD" \
      --header "Authorization: Bearer $API_KEY" \
      --header 'Content-Type: application/json' \  
      ```
      </CodeGroup>
