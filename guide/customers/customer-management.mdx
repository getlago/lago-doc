...
<Tabs>
  <Tab title="Dashboard">
      ...
      3. Enter the customer's name (255 max char.) and external ID (i.e. unique ID as defined in your
         backend system);
...
<Warning>
  You cannot change the `external_id` of customers after an object has been
  assigned to them (i.e. plans, coupons, one-off invoices, wallet and credits).
</Warning>

## Currency and billing entity
The `currency` and [billing entity](/guide/billing-entities) defined on a customer act as **defaults**.

When creating a billing object for the customer (subscription, coupon, one-off invoice, wallet), you can select a different currency and billing entity. The related invoices are issued in the currency and by the billing entity selected on the object.

Note that coupons and prepaid credits only apply to invoices sharing the same currency.
...
## Monitor the customer's billing status
...
<Tabs>
  ...
  <Tab title="API">
   You can retrieve the customer's gross revenue via the API:
      <CodeGroup>
      ...
      You can also retrieve the customer's overdue balance via the API:
      <CodeGroup>
