...
Lago retries failed deliveries up to 3 times. On self-hosted instances, set the retry count with `LAGO_WEBHOOK_ATTEMPTS` and the request timeout with `LAGO_WEBHOOK_TIMEOUT_SECONDS` (default: 30 seconds).
