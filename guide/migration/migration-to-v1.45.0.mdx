...
| `LIMIT` | _(all)_ | Maximum number of **customers** to process. The task iterates over distinct customers — all wallets belonging to a customer are always processed atomically as a group. When set, the task prints a **next cursor** value (see `CURSOR`). |
| `CURSOR` | _(first customer)_ | Start processing from a specific customer ID (inclusive). Must be a valid UUID. When omitted, the task defaults to the first customer ID in scope. When `LIMIT` is set, the task prints a **next cursor** value that can be passed as `CURSOR` on the following run to continue where the previous one left off. |
...
| `ERROR_LOG_FILE` | _(tmp dir)_ | File path for the CSV error log. A timestamped CSV file is automatically created in the system's temporary directory (e.g., `/tmp/wallet_migration_errors_20260402120000.csv`). Set this variable to override the default path. The file contains problematic wallets (dry-run) or errors (backfill). |
...
**Multiple processes with `CURSOR` + `LIMIT`** is the recommended approach for large datasets. By splitting the customer space into non-overlapping windows, you can run several independent processes in parallel — each handling a different slice. This sidesteps the GVL entirely and scales linearly with the number of processes.

To set this up, first run a dry-run with `LIMIT` to discover the cursor boundaries:

```bash
# Process 1: first 10,000 customers
DRY_RUN=false LIMIT=10000 bundle exec rails migrations:wallet_traceability
# Output: Next cursor: <cursor_A>

# Process 2: next 10,000 customers (can run in parallel with Process 1)
DRY_RUN=false LIMIT=10000 CURSOR=<cursor_A> bundle exec rails migrations:wallet_traceability
# Output: Next cursor: <cursor_B>

# Process 3: next 10,000 customers (can run in parallel)
DRY_RUN=false LIMIT=10000 CURSOR=<cursor_B> bundle exec rails migrations:wallet_traceability
# ...and so on
```

Since the task only processes non-traceable wallets and acquires customer-level advisory locks, it is safe to re-run a cursor window — already-migrated customers will be skipped automatically.

<Note>
You can discover all cursor boundaries upfront by running successive dry-runs with `LIMIT`, collecting each "Next cursor" value, and then launching all processes in parallel.
</Note>
