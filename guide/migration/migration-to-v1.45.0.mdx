...
To set this up, you simply launch the first process to discover the next cursor, then launch additional processes with the provided cursor to split the workload:
...
|---|---|---|---|
| **Decimal amount_cents** | `Decimal amount_cents on inbound TX_ID: 12.9999 (expected integer)` | Historical bug storing non-rounded decimal values instead of integers. | Update the transaction amount in the database to rounded integers. |
