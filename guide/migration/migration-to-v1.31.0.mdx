...
We're writing to inform you about important changes in Lago v1.31.0 that will enhance data consistency, application performance and prepare upcoming features.

# What are the changes?

## Filling `organization_id` on every table of the application

The `organization_id` column was recently added on every table related to an organization.
Since filling this column is a heavy task, no migration was backfilling it for existing records. It was only populated on newly created records.

As we now need to be able to rely on this column in the application, we have to make sure that every single record has an `organization_id` value.

# Why are we doing this?

These changes are made to increase the performance of the application and to ease the development on future features.
...
```
  ...
  The task will:
  - Check all database tables to identify the ones that require a migration
  - Enqueue jobs for each table needing a backfill
  ...
  The task will monitor the values and give you the number of remaining records.
  </Note>
