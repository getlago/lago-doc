...
Lago Premium organizations with the enterprise API-permissions add-on can restrict an API key by resource and mode. Permission identifiers use the machine-readable `<resource>:<mode>` format, where the mode is `read` or `write`. For example, a workload that only retrieves invoices can use `invoice:read`, while event ingestion requires `event:write`.

The [same-origin OpenAPI discovery profile](/openapi.json) declares the required permission for each representative operation in `x-lago-required-api-key-permissions` and describes the permission model on `components.securitySchemes.bearerAuth`. These are API-key permissions, not OAuth scopes, and they do not create a delegated-authorization flow.

Configure permissions when creating or editing a key under **Developers > API keys**. See [API key permissions](/guide/security/api-keys#set-api-key-permissions) for availability and operator instructions.
