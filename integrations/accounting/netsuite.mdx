...
<Steps>
  <Step title="Step 1: Create a Lago Tax Nexus">
    ...
    2. Choose a new **Country** (*⚠️ ensure it's unused by other existing tax nexuses*. It can be a new state for an existing country);
    3. Enter `Lago Tax Nexus` in the **Description** field;
    4. Create a new **Tax Agency** by clicking the **+** button. Name it `Lago Tax Agency` and assign it to the relevant parent subsidiary; and
    5. Save your changes.
