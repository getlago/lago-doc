---
title: "Xero"
description: "Sync your Lago billing data to Xero in real time."
---
...
Start by connecting your Lago instance to a new Xero connection. You can create as many Xero connections as you need.
Once a connection is live, activate the syncs and actions your use case requires.
...
- `Customers`: Sync or fetch customer data from Xero *(mandatory)*;
...
<Frame caption="Enable syncs and actions for Xero">
...
<Frame caption="Map a payment account between Lago and Xero">
...
- Any discounts on a credit note (like a coupon, for instance) are synced as line items on the Xero Credit Note.
...
If a Lago invoice is tied to a Xero invoice, Lago automatically syncs payments occurring in Lago to Xero, updating the payment status in real time of the invoice in Xero.
...
## Integration logs
Whenever an issue occurs in Xero, Lago will notify you through a [webhook message](/api-reference/webhooks/messages#param-customer-accounting-provider-error) called `customer.accounting_provider_error`.