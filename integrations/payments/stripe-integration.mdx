...
3. Click **"Stripe"** to create a connection; 
4. Give a name to this connection;
5. Assign a code for easy identification;
6. Enter your Stripe API key ([locate your API key](https://support.stripe.com/questions/locate-api-keys-in-the-dashboard)); and
...
2. Select the Stripe connected account;
3. Leave the field associated with the **Stripe customer ID** blank;
4. **Enable** the option to automatically create the customer in Stripe; and
5. Define payment method options for this customer. Possible values are `card` and `sepa_debit`.
...
1. Define Stripe as the **default payment provider**;
2. Select the Stripe connected account;
3. Provide the **Stripe customer ID**;
4. **Disable** the option to automatically create the customer in Stripe; and
5. Define payment method options for this customer. Possible values are `card` and `sepa_debit`.
