...
Lago can detect and apply European tax rates for your customers automatically.
...
To activate Lago's automatic EU tax detection:
...
These rates are labeled `automated` in Lago and follow the standard tax rates published for European countries.
Each one is prefixed with `lago_`, so you can tell them apart from the rates you create yourself.
...
<Frame caption="Tax auto-application decision tree">
...
| Spain | Ceuta | `lago_eu_es_exception_ceuta` | 51001-51005, 51070-51071, 51081 | 0% |
| Spain | Melilla | `lago_eu_es_exception_melilla` | 52000-52006, 52070-52071, 52081 | 0% |
...
| Portugal | Madeira | `lago_eu_pt_exception_madeira` | 90xx-94xx | 22% |
| Portugal | Azores | `lago_eu_pt_exception_azores` | 95xx-99xx | 18% |
| Austria | Jungholz | `lago_eu_at_exception_jungholz` | 6691 | 19% |
| Austria | Mittelberg | `lago_eu_at_exception_mittelberg` | 6991, 6992, 6993 | 19% |
| Greece | Mount Athos | `lago_eu_gr_exception_mount_athos` | 63086 | 0% |

<Info>
  **France DOM-TOM exception:** For French overseas territories (Guadeloupe, Martinique, Guyane, Réunion, Mayotte), the exception rate only applies to **B2B customers** (those with a `tax_identification_number` that passes validation). B2C customers in these territories are charged the standard French rate of 20%, in line with French tax law (CGI Articles 294–296).
</Info>
...
2. If your customer's `country` is within the European Union, Lago applies the tax rate corresponding to your customer's own country; or
