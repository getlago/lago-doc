...
If your webhook endpoint is configured correctly, your **customer, subscription, and billing data flow in real time**.
...
When `Customer Type` is set to `Individual`: Creates a Person Record type account.
...
We don't keep the data in sync from lago to Salesforce. Only initial sync is available from lago to Salesforce, check below section.
...
Note that this subscription is automatically linked to a Salesforce Account:
...
Now we can also get invoice pdf directly from Salesforce. 
...
4. Complete the Application
  ...
  <Frame caption="Apply Coupon on Account">
