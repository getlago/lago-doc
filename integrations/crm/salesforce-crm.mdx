...
3. Within the Lago Base Configuration tab:
  ...
  - Enter your Lago **API base URL** (e.g., https://api.getlago.com/). Exclude the `api/v1` at the end of the URL
  - If you want to change the API base URL to a custom one, please follow this [guide](#configure-customer-api-url);
4. **"Save and validate"** your connection; and
5. Optionally click the **"Start Sync Data"** to backfill historical data from Lago to Salesforce;
...
Salesforce Account is created first, and that the Lago Customer's external_salesforce_id is populated.

<Warning>
Billing data will not sync unless these requirements are met. 
Lago will not create an Account in Salesforce without prior specification.
</Warning>
