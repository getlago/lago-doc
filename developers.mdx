...
The [Agent Skills discovery index](/.well-known/agent-skills/index.json) publishes the [Lago billing-integration skill](/.well-known/agent-skills/lago/skill.md) with a content digest. The [A2A agent card](/.well-known/agent-card.json) and [MCP server card](/.well-known/mcp/server-card.json) expose Lago's identity, documentation tools, and read-only skill resource at predictable URLs.
