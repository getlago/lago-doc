...
<Update label="August 2026">
  ...
  **Or bill from the gateway's own books.** Point the SDK at the gateway's logs and it bills Cloudflare's metered cost per call. Not our estimate of what the call cost. The number Cloudflare charged you.
  ...
  - **The real figure, not a rate card.** Cloudflare reports an exact cost per call. On this path the SDK bills that number and skips its own price lookup entirely. The cost you bill on is the cost you were charged, before you add your own margin.
  ...
  Two limits worth knowing up front. Live cache-hit detection reads a response header, so it covers non-streaming calls on OpenAI and Anthropic clients only. Streamed calls, and Mistral, Gemini and Bedrock clients, bill a cache hit as a normal call. The log backfill catches all of them, because Cloudflare records a cache hit with zero tokens and zero cost.
