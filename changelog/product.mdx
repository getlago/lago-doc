...
<Update label="July 2026">
  ...
  - **Multi-currency customers**: assign plans denominated in different currencies to the same customer; each subscription bills in its plan's currency.
  - **Multi-entity billing**: invoice the same customer from different billing entities, without duplicate customer records.
  - **Discounts stay coherent**: coupons and prepaid credits only apply to invoices sharing their currency.
  - **Nothing changes by default**: if you never override, objects keep inheriting the customer's currency and billing entity, exactly as before.
