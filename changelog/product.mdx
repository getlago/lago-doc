...
>
  ...
  <Frame>
    <img src="/changelog/images/20260903-lago-cli.webp" alt="A terminal running lago customers create, printing the identifier block, then lago invoices list with JSON output." loading="lazy" />
  </Frame>
  ...
  Install with `go install github.com/getlago/lago-cli/cmd/lago@latest`, then `lago init` validates the key before writing a profile. The CLI sends no telemetry.
