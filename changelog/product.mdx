...
<Update label="July 2026">
  ...
  Until now, a customer was locked to a single currency and a single billing entity. The first object you assigned froze the currency for good, and billing the same customer from another entity meant duplicating the customer record.

  That limitation is gone. The currency and billing entity on a customer are now **defaults, not constraints**. When you create a billing object — a subscription, a coupon, a one-off invoice, a wallet — you can select a different currency and a different billing entity. The resulting invoices are issued in the currency, and by the billing entity, selected on the object.

  Why it matters: your customer is one company, and your billing should see them that way. A global account billed in EUR in Europe and USD in the US, or invoiced from two of your legal entities, no longer requires duplicate customer records — one profile carries the whole relationship. That's one place for settings, payment methods, and taxes, one customer to keep in sync with your CRM and accounting tools, and reporting that reflects reality: usage, revenue, and balances roll up to a single customer instead of being scattered across duplicates you have to reconcile.
