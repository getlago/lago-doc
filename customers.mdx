...
6. Select the default payment provider for this customer (optional -
   [learn more](/docs/guide/payments/overview)); and
7. Click **"Create customer"** to confirm.

Once a customer is created, you can access the customer view, where you can edit
their information.

<Warning>
  You cannot change the `external_id` of a customer after an object has been
  assigned to them (i.e. plan, coupon, add-on, wallet and credits).
</Warning>

You can also
[create and update customers](/docs/api/customers/create-update-customer) via
the API.
...
customer, which triggers a [subscription](/docs/guide/plans/subscription).

You can also apply [coupons](/docs/guide/coupons), [add-ons](/docs/guide/addons)
and [prepaid credits](/docs/guide/prepaid%5Fcredits) to a customer account.
...
start pushing [events](/docs/guide/events/ingesting%5Fevents) associated with
...
You can retrieve the customer's current usage via the API using
[this endpoint](/docs/api/customer%5Fusage/customer-usage).
...
[invoice](/docs/guide/invoicing/overview) for them at the end of the billing
period.
...
The timezone feature is only available to users with a premium license. Please
