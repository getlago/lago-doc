...
6. Select the default payment provider for this customer (optional - [learn more](/docs/guide/payments/overview)); and
...
You can also [create and update customers](/docs/api/customers/create-update-customer) via the API.

## Assign objects to a customer[](#assign-objects-to-a-customer "Direct link to heading")

The usage monitoring and billing processes start when you assign a plan to a customer, which triggers a [subscription](/docs/guide/plans/subscription).

You can also apply [coupons](/docs/guide/coupons), [add-ons](/docs/guide/addons) and [prepaid credits](/docs/guide/prepaid%5Fcredits) to a customer account.
...
When a plan that includes usage-based charges is assigned to a customer, you can start pushing [events](/docs/guide/events/ingesting%5Fevents) associated with the customer account.
...
![View of the &quot;Usage&quot; tab](/images/customer-current-usage-28893d7bd2e44ae054012ffa8e3cc69b.png)

You can retrieve the customer's current usage via the API using [this endpoint](/docs/api/customer%5Fusage/customer-usage).

## Invoicing a customer[](#invoicing-a-customer "Direct link to heading")

If a customer has an active subscription, Lago will automatically generate an [invoice](/docs/guide/invoicing/overview) for them at the end of the billing period.
