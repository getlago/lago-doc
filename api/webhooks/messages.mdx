---
title: "Messages"
description: "Here is the list of webhook messages sent by Lago."
---
...
Sent when an add-on has been applied to customer. The invoice is created
immediately after the add-on is assigned to a customer.
...
| Attributes                          | Description                                                       |
| ----------------------------------- | ----------------------------------------------------------------- |
| **credit_note** `JSON` Not nullable | [Credit note object](/docs/api/credit%5Fnotes/credit-note-object) |
...
This webhook is sent when a customer is successfully created in the payment
provider's application.

This allows you to retrieve the identifier assigned to the customer by the
...
This webhook is sent when there is an error related to the creation of a
customer in the payment provider's application.
...
Includes the URL that should be used to set up the
[direct debit mandate](/docs/guide/payments/gocardless-integration#direct-debit).
...
Usually occurs when no valid payment method is defined for the customer in the
payment service provider's application.
...
This webhook is sent when there is an error related to a credit note refund of a
payment previously created on a payment provider.
