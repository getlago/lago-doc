...
description:
  "This guide explains how Lago ingests usage-based events coming from your
  application."
---
...
[Billable metrics](/docs/guide/billable-metrics/overview) created from the UI**.
If you don't understand the concept of Billable metrics, we recommend you to
read it first.
...
```json
{
  ...
  "properties": {
    // (Optional) Custom variables defined as properties
    "custom_field": 12
  }
}
```
...
<Tip>
**A good practice is to send the id of the transaction coming from your
backend**.
...
`MAX` and `COUNT DISTINCT`. Event properties can be `strings`, `integers`,
...
**[Plans](/docs/guide/plans/overview)**.
