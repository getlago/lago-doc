...
A [subscription](/docs/api/subscriptions/subscription-object) is created when a plan is assigned to a customer. You can assign a plan to a customer at any time (i.e. when the customer record is created or later on).
...
7. Choose whether the subscription should be renewed at the beginning of the period or on its anniversary date (see [below](/docs/guide/plans/subscription#billing-cycles)); and
...
When a subscription is active, Lago will automatically generate invoices for the customer according to the [plan model](/docs/guide/plans/plan-model). It will also start monitoring the customer's consumption, which means that you can start pushing [events](/docs/api/events/metered-event) related to this subscription.
...
**Example 1:** Start date in the past and subscription fee to be paid in advance![Illustration for start date in the past and subscription fee in advance](/images/subscription-past-advance-0bedeeb9f9918608568ea7db6479e4ef.png)

**Example 2:** Start date in the past and subscription fee to be paid in arrears![Illustration for start date in the past and subscription fee in advance](/images/subscription-past-arrears-b4f22483b4ae3a358c0b27305e49a766.png)
...
**Example 3:** Start date in the future and subscription fee to be paid in advance![Illustration for start date in the future and subscription fee in advance](/images/subscription-future-advance-e0c8d632579fe11dfc530987d86ca211.png)

**Example 4:** Start date in the future and subscription fee to be paid in arrears![Illustration for start date in the future and subscription fee in arrears](/images/subscription-future-arrears-8b99c7c98e2f8223d6282fd5f47a665a.png)

info

It is possible to update the start date of a pending subscription via the user interface (click the **ellipsis icon**, then **"Edit subscription"**) or [via the API](/docs/api/subscriptions/update-subscription).
...
2. You must specify the `external_subscription_id` for each event or create [batch events](/docs/api/events/create-batch-event).
