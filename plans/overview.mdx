...
Note that you don't need to define a Billable metric to create a Plan. However, if you have usage feature to charge, the price of your Billable metrics is defined inside each Plan. Make sure to understand [how to create a Billable metric](/docs/guide/billable-metrics/overview) first.
...
1. [The Plan basic informations](#plan-structure)  
   ...
   * A Plan `code`  
   * A Plan `description`
2. [The Plan model](/docs/guide/plans/plan-model)  
   * A Plan `interval`  
   * A Plan base `amount` with the `currency`  
   * Boolean to define if the Plan is `pay in advance`  
   * A Plan `trial period` in days
3. [The additional charges](/docs/guide/plans/charges) for this Plan (any Billable metrics that have been created)  
   * A Charge `model`  
...
Plans can be edited and deleted only if they are not linked to a Customer (meaning there is no active [subscription](/docs/guide/plans/subscription) for this Plan).
