...
![Illustration of the grace period](/images/grace-period-timeline-299de2425036f803f20825e8a42ab401.png)
...
The creation of a `draft` invoice triggers the `invoice.drafted` [webhook](/docs/api/webhooks/messages).
...
In order to add usage to an invoice that is in `draft` status, the `timestamp` of the [events](/docs/guide/events/ingesting%5Fevents#send-usage-measurements-to-lago) must be within the billing period. Consider the following example:
...
![Draft invoice in the Lago app](/images/grace-period-draft-invoice-0a9ccbd1d7dc1b87faeb5a44abf484e1.png)
...
![Grace period in the settings of the app](/images/grace-period-organization-eabdac12832b76fd4d75c1a266244339.png)

You can modify the organization's grace period at any time through the user interface or [the API](/docs/api/organizations/update-organization).
...
You can modify the customer's grace period at any time through the user interface or [the API](/docs/api/customers/create-update-customer).

![Grace period in the customer view](/images/grace-period-customer-19e2d76ad85f4a1af86c6d8fd0f38715.png)
